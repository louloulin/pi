...
| `setStatus` | Forwarded to the injected `UiRegionHost` as one synchronous `host_ui_region("setStatus", { key, text })` call; `text` is `null` for upstream's `undefined` clear. The shim keeps the map too, so `footerData.getExtensionStatuses()` returns a copy of it to a custom footer. Without a region host the call is reported as a denial and nothing is queued. |
