...
| `addAutocompleteProvider(factory)` | Registers the factory in the shim's wrapper chain and forwards the chain's deduplicated `triggerCharacters` to the editor (LUM-1448). The factory receives `current` — the built-in `CombinedAutocompleteProvider`, reached from JS through the synchronous `host_ui_autocomplete` import — and returns `{ triggerCharacters, getSuggestions, applyCompletion, shouldTriggerFileCompletion }`. ⚠️ Only synchronous callbacks are served: a Promise-returning callback is not awaited, that call falls back to the built-in provider, and a warning is reported once. The Rust→JS entry point is `_pi_autocomplete_call(op, json)`. |
| `setStatus` | Forwarded to the injected `UiRegionHost` as one synchronous `host_ui_region("setStatus", { key, text })` call; `text` is `null` for upstream's `undefined` clear. The shim keeps the map too — **session-scoped**, not per-`ctx` (LUM-1490) — so `footerData.getExtensionStatuses()` returns a copy of it to a custom footer no matter which handler set it. Without a region host the call is reported as a denial and nothing is queued. |
| `footerData` (3rd `setFooter` factory argument) | Upstream's `ReadonlyFooterDataProvider` (`footer-data-provider.ts:387-390`), complete: `getGitBranch()` (**LUM-1490**, host→JS query), `getAvailableProviderCount()` (**LUM-1490**), `getExtensionStatuses()` (the session map above) and `onBranchChange(cb)` (**LUM-1490**, returns the unsubscribe function). The first two are the one **host → JS** direction of the bridge: the interactive driver pushes a snapshot (`JsExtensionHost::sync_footer_data`) and the shim queries it back synchronously through `host_ui_region("footerData", { field })`, so a render-time read never crosses an await. `getGitBranch()` answers `null` outside a repository and on a detached HEAD — the same contract `pi-coding-agent`'s built-in footer implements — and `null` (not `undefined`) is also the untouched pre-resolve answer. `onBranchChange` fires on a real `HEAD` transition only: never on the initial snapshot, and subscribers live at session scope, so a `ctx` built by a later event cannot lose them. The branch is re-read every frame, so a `git checkout` in another terminal reaches a custom footer without a restart. |
| `setTitle` | Forwarded to the injected `UiRegionHost` as one synchronous `host_ui_region("setTitle", { title })` call. The host sanitises control characters and hands the title to the TUI, which queues it; the driver writes upstream's OSC 0 sequence (`\x1b]0;…\x07`) on the tty outside the frame buffer. Without a region host the call is reported as a denial and nothing is queued. The interactive mode also sets its own title (`<app> - <session name> - <cwd basename>`) whenever session facts change, which replaces a title an extension set earlier — upstream behaves the same way. |
| `setEditorText`, `setHiddenThinkingLabel`, `setWorkingIndicator`, `setWorkingVisible`, `setWorkingMessage`, `setTheme` | No-op with a one-time warning notification. Accepting the call lets extensions that configure them at load time still load; the value is inert because there is no channel for it. |
