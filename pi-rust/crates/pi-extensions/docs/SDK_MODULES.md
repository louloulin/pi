...
| `setTitle`, `setEditorText`, `setHiddenThinkingLabel`, `setWorkingIndicator`, `setWorkingVisible`, `setWorkingMessage`, `setTheme` | No-op with a one-time warning notification. Accepting the call lets extensions that configure them at load time still load; the value is inert because there is no channel for it. |
