...
| Provider registration                   | ⚠️ Partial     | The extension-side `@earendil-works/pi-ai/compat` registry (own `streamSimple` + event stream) works, and the two built-in provider factories reach the host's Anthropic / OpenAI-Responses adapters (LUM-1180). `pi.registerProvider(...)` — the host bridge that would let the agent *use* an extension provider — is still out of scope, and the eight remaining lazy api families have no host adapter. |
