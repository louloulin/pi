...
| `ctx.ui.setTitle` / `setEditorText` / `setTheme` / … | — | The remaining display methods stay inert no-ops with a one-time warning notification. See [`docs/SDK_MODULES.md`](SDK_MODULES.md). |
