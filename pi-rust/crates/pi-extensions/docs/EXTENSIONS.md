...
| `ctx.ui.setWidget` / `setHeader` / `setFooter` / `setEditorComponent` | `host_ui_region("setWidget"/"setHeader"/"setFooter"/"setEditorComponent", json)` | Installs the region component, or clears it for `null` / `undefined`. The remaining display methods (`setStatus`, `setTitle`, `setEditorText`, `setTheme`, …) stay inert no-ops with a one-time warning notification. See [`docs/SDK_MODULES.md`](SDK_MODULES.md). |
...
| `@earendil-works/pi-ai/compat`          | ✅ Subset      | The pure-JS provider registry (`registerApiProvider` / `unregisterApiProviders` / `getApiProvider(s)` / `stream(Simple)` / `complete(Simple)`) and the event-stream factory, plus (LUM-1180) the built-in `anthropicMessagesApi` / `openAIResponsesApi` factories and `registerBuiltInApiProviders` / `resetApiProviders`, which stream from the host's own `AnthropicProvider` / `OpenAiResponsesProvider` across the `host_pi_ai_stream_*` bridge. The other eight upstream lazy apis (e.g. `googleGenerativeAIApi`, `openAICompletionsApi`) stay `ERR_PI_SDK_UNIMPLEMENTED`. See [`docs/SDK_MODULES.md`](SDK_MODULES.md) for the custom-header / thinking-block / cost divergences. |
...
| Custom editor / footer / header / widgets | ✅ Supported   | `ctx.ui.custom` / `setWidget` / `setHeader` / `setFooter` / `setEditorComponent` render through the `UiRegionHost` bridge (`pi-coding-agent` injects the TUI adapter; LUM-1190). The remaining status-line `set*` methods stay inert no-ops so extensions that configure them at load time still load. |
| Provider registration                   | ✅ Subset      | `pi.registerProvider(name, config)` registers the declarative form (`baseUrl` / `apiKey` / `api` / `models`) and `pi.unregisterProvider(name)` removes it; the application layer turns each entry into a `ProviderRouter` adapter plus model-catalog entries, so the agent can stream from the extension's provider. Supported `api` values: `anthropic-messages`, `openai-responses`, `openai-completions`, `google-generative-ai` (anything else throws with the supported set). **Not done in this slice:** the native `Provider` object overload (throws `ERR_PI_SDK_UNIMPLEMENTED`), the `oauth` block, `streamSimple` handlers, `headers` / `authHeader` / `refreshModels`, and the `!command` apiKey form (never executed). The extension-side `@earendil-works/pi-ai/compat` registry is unchanged (LUM-1180). |
