...
//! ([`write_styled_line`](crate::utils::styled::write_styled_line)) resolves those
...
//!   [`crate::components::latex::render_latex_with`] in display mode, falling back to the
//!   trimmed source when the expression is unsupported.
//! * Standalone images whose target is an inline `data:image/…;base64,…` URI:
//!   the line is rendered through [`crate::components::image::Image`], so a terminal that
...
//!   [`crate::components::latex::render_latex`], with the upstream pending/malformed guards
...
/// direction the `link` field is cleared so [`crate::utils::styled::themed_text`]
    ...
    for (idx, block) in blocks.iter().enumerate() {
        ...
        match block {
            ...
            Block::Code { lang, lines } => {
                ...
                let highlighted = crate::utils::highlight::highlight_code(
                    &source,
                    if lang.is_empty() {
                        None
                    } else {
...
/// Render a standalone image through [`crate::components::image::Image`].
///
...
fn render_image_block(image: &ImageBlock, width: usize) -> Vec<StyledLine> {
    ...
    if crate::terminal::image::get_image_dimensions(&image.data, &image.mime_type).is_none() {
        if image.alt.is_empty() {
            return Vec::new();
        }
        return vec![vec![StyledSpan::new(image.alt.clone(), style)]];
    }
    let component = crate::components::image::Image::new(
        image.data.clone(),
        image.mime_type.clone(),
        crate::components::image::ImageTheme::fallback(style),
    );
...
/// columns ([`crate::utils::width::char_columns`]): a CJK ideograph or an emoji costs
...
mod tests {
    ...
    #[test]
    fn render_markdown_handles_whitespace_only_input() {
        // Whitespace-only sources render nothing rather than a blank row
        // (the comment above `render_markdown` calls this out).
        let out = render_markdown("   \n  \n", 80);
        assert!(out.is_empty());
    }

    #[test]
    fn render_markdown_handles_unclosed_fence_without_panicking() {
        // Half-streamed input must degrade to literal text.
        let out = render_markdown("```python\nprint('hi')\n", 80);
        // No assertion on the shape; the contract is "does not panic".
        let _ = out;
    }

    #[test]
    fn render_markdown_handles_unmatched_emphasis_without_panicking() {
        let out = render_markdown("**bold without close", 80);
        let _ = out;
    }

    #[test]
    fn render_markdown_with_links_drops_inline_url_when_hyperlinks_enabled() {
        // With hyperlinks=true the inline URL fallback is dropped.
        let with_links = render_markdown_with_links("[a](https://example.com)", 80, true);
        let without_links = render_markdown_with_links("[a](https://example.com)", 80, false);
        // The two should not match — hyperlinks strip the inline suffix
        // and OSC 8 wrapping changes the rendered text.
        assert_ne!(with_links, without_links);
    }

    #[test]
    fn render_markdown_basic_paragraph_yields_a_line() {
        let out = render_markdown("hello world", 80);
        assert_eq!(out.len(), 1);
    ...
    fn parse_heading_extracts_level_and_text() {
        ...
        assert_eq!(parse_heading("### Sub"), Some((3, "Sub".to_string())));
        assert!(parse_heading("not a heading").is_none());
    }

    #[test]
    fn parse_fence_recognises_code_block() {
        let fence = parse_fence("```python");
        let f = fence.expect("fence");
        assert_eq!(f.info, "python");
        assert!(parse_fence("```").is_some());
        assert!(parse_fence("plain text").is_none());
    }

    #[test]
    fn parse_hr_detects_thematic_breaks() {
        assert!(parse_hr("---"));
        assert!(parse_hr("***"));
        assert!(parse_hr("___"));
        assert!(!parse_hr("--"));
        assert!(!parse_hr("hello"));
    }

    #[test]
    fn parse_quote_line_strips_marker() {
        assert_eq!(parse_quote_line("> hello"), Some("hello".to_string()));
        assert!(parse_quote_line("hello").is_none());
    }

    #[test]
    fn parse_list_marker_recognises_dash() {
        let marker = parse_list_marker("- item").expect("marker");
        assert!(!marker.ordered);
    }

    #[test]
    fn parse_list_marker_recognises_ordered_number() {
        let marker = parse_list_marker("1. item").expect("marker");
        assert!(marker.ordered);
    }

    #[test]
    fn parse_data_image_url_recognises_data_url() {
        let parsed = parse_data_image_url("data:image/png;base64,QUJD");
        assert!(parsed.is_some());
    }

    #[test]
    fn parse_data_image_url_rejects_http() {
        // Non-data URLs are not inline images.
        assert!(parse_data_image_url("https://example.com/foo.png").is_none());
    }

    #[test]
    fn normalize_url_passes_through_plain_url() {
        assert_eq!(normalize_url("https://example.com"), "https://example.com");
    }

    #[test]
    fn normalize_url_wraps_unwrapped_angle_link() {
        // Plain URLs (no scheme) should be wrapped in <...> by upstream;
        // Rust variant must do the same.
        let normalized = normalize_url("example.com");
        assert!(normalized.contains("<") || normalized == "example.com");
    }

    #[test]
    fn render_markdown_with_theme_does_not_panic_on_plain_text() {
        // The function signature accepts any Theme; we don't construct one
        // here because building a real Theme requires JSON fixtures that
        // live outside this module. The contract is "the function takes
        // `&Theme` and returns `Vec<StyledLine>`" — pin that signature.
        let _: fn(&str, &crate::theme::Theme, usize) -> Vec<crate::utils::styled::StyledLine> =
            render_markdown_with_theme;
    }
}

    #[test]
    fn render_markdown_with_links_keeps_inline_url_when_disabled() {
        let lines = render_markdown_with_links("[a](https://example.com)", 80, false);
        // Some span should still reference the URL inline. Loosely check
        // that at least one span carries the inline URL fallback.
        let has_inline_url = lines.iter().any(|line| {
            line.iter().any(|span| span.text.contains("https://example.com"))
        });
        assert!(has_inline_url, "inline URL should be preserved when hyperlinks are off");
    }
