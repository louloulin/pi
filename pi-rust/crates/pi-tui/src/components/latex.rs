...
mod tests {
    ...
    #[test]
    fn render_latex_plain_text_succeeds() {
        // The parser is permissive: plain ASCII without commands still
        // round-trips through the renderer.
        assert!(render_latex("plain text").is_some());
    }

    #[test]
    fn render_latex_returns_some_for_alpha() {
        // `\alpha` is a recognised Greek letter.
        assert!(render_latex("\\alpha").is_some());
    }

    #[test]
    fn render_latex_returns_some_for_simple_fraction() {
        // `\frac{1}{2}` is a stacked fraction.
        let out = render_latex("\\frac{1}{2}");
        assert!(out.is_some());
    }

    #[test]
    fn render_latex_inline_vs_display_fraction_match_basic() {
        let inline = render_latex_with("\\frac{1}{2}", false);
        let display = render_latex_with("\\frac{1}{2}", true);
        // Both succeed; we don't assert the exact glyph because the
        // renderer is glyph-table-dependent — we only need a regression
        // pin that the display/inline branches both complete.
        assert!(inline.is_some());
        assert!(display.is_some());
    }

    #[test]
    fn render_latex_rejects_unterminated_command() {
        // An unterminated brace means the parser cannot reach end-of-source
        // cleanly, so it should return None.
        assert!(render_latex("\\frac{1}{2").is_none());
    }

    #[test]
    fn render_latex_rejects_unknown_command() {
        assert!(render_latex("\\notarealcommand{x}").is_none());
    }

    #[test]
    fn render_latex_handles_sum_with_limits() {
        assert!(render_latex("\\sum_{i=1}^{n} x_i").is_some());
    }

    #[test]
    fn render_latex_handles_sqrt() {
        assert!(render_latex("\\sqrt{x}").is_some());
    }

    #[test]
    fn render_latex_display_mode_dedents_indented_layout() {
        // A stacked fraction with display mode should at least produce
        // a Some result and contain a numeric character (the operands).
        let out = render_latex_with("\\frac{a}{b}", true);
        if let Some(s) = out {
