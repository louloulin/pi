...
use crate::utils::fuzzy::fuzzy_rank;
use crate::components::selector::SelectListRow;
...
mod tests {
    ...
    #[test]
    fn autocomplete_item_value_and_label_round_trip() {
        let item = AutocompleteItem::new("value", "label");
        assert_eq!(item.value, "value");
        assert_eq!(item.label, "label");
        assert_eq!(item.description, None);

        let item = item.with_description("desc");
        assert_eq!(item.description.as_deref(), Some("desc"));
    }

    #[test]
    fn slash_command_default_has_empty_argument_hint() {
        let cmd = SlashCommand::new("foo");
        assert_eq!(cmd.name, "foo");
        assert!(cmd.argument_hint.is_none());
    }

    #[test]
    fn slash_command_with_argument_hint_stores_it() {
        let cmd = SlashCommand::new("foo").with_argument_hint("<x>");
        assert_eq!(cmd.argument_hint.as_deref(), Some("<x>"));
    }

    #[test]
    fn combined_provider_commands_returns_a_slice() {
        let provider = CombinedAutocompleteProvider::new(
            vec![SlashCommand::new("a"), SlashCommand::new("b")],
            ".",
        );
    ...
    #[test]
    fn combined_provider_base_path_returns_a_path() {
        let provider = CombinedAutocompleteProvider::new(Vec::new(), "/tmp");
        assert_eq!(provider.base_path(), std::path::Path::new("/tmp"));
    }

    #[test]
    fn triggered_provider_trigger_characters_returns_a_slice() {
        let inner: Arc<dyn AutocompleteProvider> = Arc::new(CombinedAutocompleteProvider::new(
            Vec::new(),
            ".",
        ));
    ...
    #[test]
    fn autocomplete_provider_default_should_trigger_is_true() {
        // The trait default is `true` — pin the signature for any
        // future override that breaks it.
        let _: fn(
            &dyn AutocompleteProvider,
            &[String],
            usize,
            usize,
        ) -> bool = |p, lines, line, col| {
            p.should_trigger_file_completion(lines, line, col)
        };
    }
}
