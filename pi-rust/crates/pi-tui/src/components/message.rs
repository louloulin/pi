...
pub fn tool_fold_hint(hidden: usize) -> String {
    format!(
        "… (+{hidden} lines, {})",
        crate::components::keybindings::key_hint_or("app.tools.expand", "Ctrl+O", "to expand")
    )
}
...
pub struct MessageView {
    ...
    /// [`crate::components::markdown::render_markdown`] instead of the plain-text
    ...
    /// [`crate::terminal::capabilities::hyperlinks_supported`] via
    /// `AppConfig::hyperlinks`.
    hyperlinks: bool,
...
impl MessageView {
    ...
    /// ([`crate::utils::styled::write_styled_line_ellipsized`]), so a queued draft
    ...
    /// rendered by [`crate::components::markdown::render_markdown`] instead and carries the
...
fn markdown_lines(
    ...
    let lines = crate::components::markdown::render_markdown_with_links(body, width, hyperlinks);
    if lines.is_empty() {
        return vec![vec![StyledSpan::new(prefix, prefix_style)]];
    }
    let verbatim = image_row_mask(&lines);
...
fn image_row_mask(lines: &[StyledLine]) -> Vec<bool> {
    let is_image: Vec<bool> = lines
        .iter()
        .map(|line| crate::terminal::image::is_image_line(&plain_text(line)))
        .collect();
