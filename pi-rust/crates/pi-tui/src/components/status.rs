...
use crate::components::loader::format_elapsed;
...
use crate::utils::width::{columns, prefix_columns};
...
/// ([`crate::components::loader::Spinner`]); this struct is the immutable snapshot the
/// render path consumes, so the status bar itself stays stateless.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct BusyIndicator {
    /// The spinner glyph to draw (see [`crate::components::loader::SPINNER_FRAMES`]).
    pub frame: char,
...
mod tests {
    use super::*;
    use crate::components::loader::SPINNER_FRAMES;
    ...
    fn the_busy_segment_advances_through_the_frame_table() {
        let bar = StatusBar::new();
        let mut spinner = crate::components::loader::Spinner::new();
        let mut data = StatusData::new("m", "s");
    ...
    fn a_budgeted_bar_is_laid_out_by_the_same_span_rules_as_a_fitting_one() {
        ...
        for width in 1u16..=120 {
            ...
            assert_eq!(
                crate::utils::hyperlink::strip_ansi(&bar.render_themed(&data, width, &styles)),
                plain,
