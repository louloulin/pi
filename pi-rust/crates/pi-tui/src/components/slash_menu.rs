...
///
/// When ellipsizing, the result is `w + 1` cells: `w` cells of content
/// (with the last cell reserved for the ellipsis character) followed
/// by a single trailing space for visual separation from the next
/// column.
fn pad_or_ellipsize(s: &str, w: usize) -> String {
    ...
    let mut used = 0;
    for ch in s.chars() {
        let cw = crate::utils::width::char_columns(ch);
    ...
    out.push('…');
    // Pad to `w` cells before appending the trailing space.
    let ow = columns(&out);
    if ow < w {
...
mod tests {
    ...
    #[test]
    fn name_column_width_returns_minimum_for_empty_list() {
        // An empty list falls back to the minimum width (14).
        assert_eq!(name_column_width(&[]), 14);
    }

    #[test]
    fn name_column_width_uses_widest_usage_clamped() {
        // The function reads the `usage` field (not `name`) and clamps
        // the result between 14 and 26.
        let narrow = vec![SlashMenuEntry::new("a", "/a", "tiny")];
        // usage is 2 cells, but the function clamps to a minimum of 14.
        assert_eq!(name_column_width(&narrow), 14);

        let wide = vec![SlashMenuEntry::new(
            "a",
            "/command <very-long-argument-list>",
            "wide usage",
        )];
        // Wide usage is wider than 26 — clamp at 26.
        assert_eq!(name_column_width(&wide), 26);
    }

    #[test]
    fn entry_with_skill_sets_the_flag() {
        let entry = SlashMenuEntry::new("x", "/x", "").with_skill(true);
        assert!(entry.is_skill);
    }

    #[test]
    fn entry_with_completion_sets_the_flag() {
        let entry = SlashMenuEntry::new("x", "/x", "").with_completion(true);
        assert!(entry.has_completion);
    }

    #[test]
    fn pad_or_ellipsize_handles_empty_input() {
        assert_eq!(pad_or_ellipsize("", 4), "    ");
    }

    #[test]
    fn pad_or_ellipsize_preserves_content_when_wider() {
        // When the column is wider than the string, it's padded on the
        // right with spaces.
        assert_eq!(pad_or_ellipsize("ab", 5), "ab   ");
    }
}
