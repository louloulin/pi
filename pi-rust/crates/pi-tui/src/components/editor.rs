...
//! registry ([`crate::components::keybindings::get_keybindings`]) with
...
//! [`crate::components::keybindings::tui_default_keybindings`], i.e. the chords the
...
//! [`crate::utils::visual_text::VisualLayout`], the same layout [`crate::Prompt`]
...
use crate::components::autocomplete::{AutocompleteItem, AutocompleteProvider};
use crate::core::input_parse::{InputEvent, Key, KeyCode};
use crate::components::keybindings::{get_keybindings, KeybindingsManager};
use crate::components::kill_ring::{KillDirection, KillRing};
use crate::components::selector::{select_list_row_spans, select_list_visible_range, SelectorLayout};
use crate::utils::styled::{plain_text, SpanStyle, StyledLine, StyledSpan};
use crate::theme::ThemeColor;
use crate::components::undo_stack::UndoStack;
use crate::utils::visual_text::VisualLayout;
use crate::utils::word_navigation::{
...
impl Editor {
    ...
    /// placeholders. Chips always render as their label — a degraded
    /// recall (sentinel count and image count disagree) keeps the
    /// sentinels in the buffer so the labels still draw, even after the
    /// attachments themselves are dropped (see `composer_history`'s
    /// `a_raw_buffer_that_disagrees_with_the_images_degrades_to_text`).
    pub fn display_text(&self) -> String {
    ...
    pub fn push_history_entry(&mut self, entry: HistoryEntry) {
        ...
        // `[Image #N]` on recall; drop the mismatched payloads but keep the
        // sentinels so the labels still render — the buffer is the source of
        // truth for `display_text`, and stripping the sentinels would hide
        // chips the user *did* see at submission time.
        if entry.images.len() != entry.sentinels() {
            entry.images.clear();
        }
        ...
        // Capture the display text *before* moving the entry into the deque,
        // so chip sentinels survive across sessions — the in-memory entry
        // keeps the attachments, but the cross-session file only carries
        // the text (codex parity). When the in-memory entry has no
        // attachments (mismatch above, or no chips to begin with), the
        // persisted row still records the labels via `expand_chips`.
        let persisted = if entry.images.is_empty() {
            expand_chips(&entry.text)
        } else {
            entry.display_text()
        };
        ...
        if let Some(path) = self.history_path.as_deref() {
            let _ = crate::app::history_store::append(path, &persisted);
        }
    }
    ...
    fn load_persisted_history(&mut self, path: &Path) {
        for text in crate::app::history_store::load(path).into_iter().rev() {
    ...
    /// Drop in-memory history entries (the composer's view).
///
/// The cross-session file is **kept**: `/clear` resets the composer's
/// view of what it can recall, but the next launch should still see the
/// rows the user submitted. The slash command that wipes the file lives
/// at [`Editor::clear_persistent_history`] and is wired to
/// `/clear-history`.
...
/// Drop both the in-memory entries and the cross-session file.
///
/// The slash command `/clear-history` is the only caller: a
/// `/clear-history` that left the file behind would resurrect the
/// same rows on the next launch. The file delete is best-effort — a
/// read-only file is fine to leave in place.
pub fn clear_persistent_history(&mut self) {
        self.clear_history();
        if let Some(path) = self.history_path.as_deref() {
            let _ = crate::app::history_store::clear_path(path);
        }
    }
    ...
    /// newest entry appears most recently. The store's path is also wired as
    /// the cross-session file, so subsequent [`Editor::push_history_entry`]
    /// calls append to the same file a fresh process would read from.
    ///
    /// The in-memory window is capped at `store.limit()`, matching the file's
    /// cap so the composer cannot offer a wider history than what survived
    /// the trim.
    pub fn set_history_store(&mut self, store: &crate::app::history_store::HistoryStore) {
        self.history_path = Some(store.path().to_path_buf());
        let limit = store.limit();
        for text in crate::app::history_store::load(store.path()).into_iter().rev() {
...
///
/// With nothing previewed (`selected.is_none()`) this is a no-op: the search
/// stays open so the user can keep refining the query or cancel with `Esc` —
/// Enter on a miss is not an action.
pub fn accept_history_search(&mut self) -> EditorAction {
    ...
    if search.selected.is_none() {
        // No preview to accept: keep the search open so the user can
        // refine the query (or cancel with Esc) — Enter on a miss is not
        // an action.
        return EditorAction::None;
    }
    let selected_idx = search.matches[search.selected.unwrap()];
    ...
    let preview_images = self
        ...
        .unwrap_or_default();
    self.history_search = None;
    self.buffer = preview_text;
    self.images = preview_images;
    self.cursor = self.buffer.len();
    self.last_action = LastAction::Other;
    EditorAction::Changed
}
    ...
    ///
    /// Used when a paste burst is confirmed ([`crate::core::input_parse::PasteBurst`]):
    ...
    /// [`crate::utils::visual_text::VisualLayout::cursor_at`]).
    ...
    fn matches_app_exit(kb: &KeybindingsManager, key: &Key) -> bool {
        crate::components::keybindings::matches_with_fallback(
            kb,
            &InputEvent::Key(*key),
            "app.exit",
            &["ctrl+d"],
...
mod tests {
    ...
    fn manager_with(entries: &[(&str, &[&str])]) -> KeybindingsManager {
        let mut config = crate::components::keybindings::KeybindingsConfig::new();
        for (id, keys) in entries {
            config.set(*id, keys.iter().copied());
        }
        KeybindingsManager::new(crate::components::keybindings::tui_default_keybindings(), config)
    }
    ...
    fn history_chord_closes_the_autocomplete_dropdown() {
        ...
        ed.set_autocomplete_provider(Arc::new(
            crate::components::autocomplete::CombinedAutocompleteProvider::new(
                vec![crate::components::autocomplete::SlashCommand::new("help")],
                ".",
            ),
        ));
        ed.insert_str("/he");
