...
//! * the newest [`crate::components::editor::HISTORY_LIMIT`] entries win, matching the
//!   in-memory cap.
//!
...
use crate::components::editor::HISTORY_LIMIT;
...
/// Best-effort delete of a history file (used by `/clear-history`).
///
/// Missing file is not an error — the user only promised to clear, not to
/// leave evidence they ever ran it.
pub fn clear_path(path: &Path) -> std::io::Result<()> {
...
///
/// `ts` is a Unix timestamp in milliseconds at the moment the prompt was
/// appended; downstream tooling (and the test that requires every row
/// carries one) reads it back as a `u64`.
...
pub fn load(path: &Path) -> VecDeque<String> {
    ...
    for line in BufReader::new(file).lines() {
        ...
        let Some(text) = parse_line(&line) else {
            // Skip the corrupt line but flag the file for compaction so the
            // bad row does not stick around forever.
            dropped = true;
            continue;
        };
...
/// Current Unix epoch in milliseconds. Wrapped so tests can override the
/// clock if they ever need deterministic timestamps.
fn unix_millis() -> u64 {
