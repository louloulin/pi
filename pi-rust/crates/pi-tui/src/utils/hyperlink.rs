...
///
/// Thin alias for [`crate::terminal::capabilities::hyperlinks_supported`]
/// — the value lives in the single cached
/// [`crate::terminal::image::TerminalCapabilities`] record, so there is
/// only ever one answer no matter which accessor a caller uses. Tests
/// and embeds that need a specific value should override the record
/// (see [`crate::terminal::capabilities::set_overrides`]) rather than
/// mutating environment variables.
pub fn supports_hyperlinks() -> bool {
    crate::terminal::capabilities::hyperlinks_supported()
}
