...
//!   either is CJK ([`crate::utils::width::is_cjk_break`]).
...
use crate::utils::width::{char_columns, is_cjk_break};
...
impl VisualLayout {
    ...
    pub(crate) fn cursor_at(&self, row: usize, column: usize) -> usize {
        ...
        if column >= len {
            // Click past the last character of this row: return the absolute
            // character offset just past the last character.  `source.last()`
            // is the absolute offset of the last char in this row; +1 moves
            // past it.
            return vrow.source.last().copied().unwrap_or(0) + 1;
        }
        // `column` is a display column (terminal position).  `source[i]` is
        // the absolute display character offset of the character at column `i`
        // in this row — `source` already stores the offset into the draft,
        // so the result is `source[i]` directly. Binary search: find the
        // character that STARTS at or before `column`.  `binary_search(&column)`
        // returns `Ok(i)` when `source[i] == column`, or `Err(i)` as the
        // insertion point when `column` falls between character starts.  In
        // both cases `source[i]` is the absolute character offset — the
        // character at `source[i]` starts at or before `column`.
...
/// columns ([`crate::utils::width::char_columns`]); positions stay in character
/// offsets.
...
/// Columns a string occupies; see [`crate::utils::width::columns`].
#[cfg(test)]
pub(crate) use crate::utils::width::columns as display_width_of;
