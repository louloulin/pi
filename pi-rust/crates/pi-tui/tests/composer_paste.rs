...
fn a_recalled_history_entry_can_still_expand_its_marker() {
    ...
    // What `App::submit` records for the driver's turn: the submitted text
    // plus the raw buffer (marker included), so a recall restores the marker
    // rather than 12 flattened lines.
    assert_eq!(submission.raw_text.as_deref(), Some("[paste #1 +12 lines]"));
    app.prompt_mut().push_history_entry(
