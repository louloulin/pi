...
pub struct InteractiveOptions {
    ...
    /// Extension-installed shortcut registry. Populated from
    /// [`crate::extensions::ui_bridge::TuiRegionHost::extension_shortcuts`]
    /// whenever the TUI host is wired. Plan §5 P0-3.
    pub extension_shortcut_registry:
        Option<Arc<crate::extensions::extension_shortcuts::ExtensionShortcutRegistry>>,
    ...
    /// Credential store the `/login` command writes OAuth credentials
    /// into. The interactive driver is the only place the OAuth flow
    /// runs, so the store is plumbed in at this layer instead of being
    /// recovered by downcasting [`stream_fn`]. Production wires the
    /// store the [`ProviderRouter`](crate::provider::ProviderRouter)
    /// owns; tests inject a fresh in-memory store.
    pub credential_store: Arc<dyn pi_ai::auth::CredentialStore>,
...
impl Default for InteractiveOptions {
    fn default() -> Self {
        Self {
            ...
            extension_ui: None,
            extension_shortcut_registry: None,
...
pub(super) enum InternalAction {
    ...
    Suspend,
    /// A chord in the global keymap matched but produced no special
    /// action: the caller has consumed the event and should fall
    /// through to the next render step without reaching the App
    /// (`keymap::dispatch_global_chord_table` returns this when
    /// `app.model.cycleForward` / `app.thinking.cycle` / etc. fire).
    None,
}

// Sub-modules (added by M1 PR1/PR2/PR3/PR4/PR5/PR6/PR7 on 2026-09-26; see scripts/architecture_no_regression.sh)
pub mod bash_runner;
...
pub mod status_bar;
use bash_runner::BashRunner;
use command_dispatch::{
    append_prompt_templates, command_result_text, extension_command_args,
    help_text_with_extensions, run_slash_command,
};
use completion::{
    extension_autocomplete_commands, extension_header_for, install_composer_autocomplete,
    install_extension_autocomplete,
};
use interactive_loop::{run_loop, RunOutcome};
use input_handlers::{
    apply_clipboard_paste, handle_dequeue, handle_follow_up, handle_submitted, quote_if_needed,
    write_exit_output,
};
use session_lifecycle::{
    attach_session, clone_into_new_session, content_text, entries_to_messages, entry_to_item,
    normalize_session_name, open_current_session, persist_session_name, resume_session,
    session_directory, set_session_name, start_new_session, write_new_session_file,
};
use session_picker::{
    begin_session_delete, build_session_selector, confirm_session_delete, handle_picker_key,
    handle_rename_dialog_key, handle_tree_label_editor_key, is_live_session, open_rename_dialog,
    open_resume_selector, picker_kind, scoped_panel_change, selected_session, session_picker_footer,
    session_picker_title, PickerKind, PickerState, SessionPickerState,
};
use status_bar::{
    available_provider_count, default_model, is_subscription_provider, sync_status_metrics,
};
...
async fn handle_input_event(
    ...
    // while no modal or overlay owns the keyboard. The actual table lives in
    // `super::keymap::dispatch_global_chord_table`, where every entry binds
    // one `app.*` id (with a default fallback chord) to the helper that runs
    // it; this file only orchestrates the surrounding flow.
    let extension_shortcut_registry = options.extension_shortcut_registry.clone();
    if let Some(action) =
        keymap::dispatch_global_chord_table(app, agent, options, bash, extension_shortcut_registry.as_deref(), &event).await
    {
        return Ok(match action {
            InternalAction::None => None,
            other => Some(other),
        });
    }
...
pub(super) async fn handle_thinking_cycle(
    app: &mut App,
    agent: &Arc<AsyncMutex<Agent>>,
...
/// `Models.getAvailable()`.
pub(super) fn open_model_selector(app: &mut App, options: &InteractiveOptions) {
...
///
pub(super) async fn cycle_model(
    app: &mut App,
    agent: &Arc<AsyncMutex<Agent>>,
    options: &InteractiveOptions,
...
/// fallback.
pub(super) fn copy_last_assistant_message(app: &mut App) {
    let Some(text) = app
        .messages()
        .items()
        .iter()
...
pub(super) fn open_tree_selector(app: &mut App, options: &InteractiveOptions) {
...
pub(super) fn open_fork_selector(app: &mut App, options: &InteractiveOptions) {
...
pub(super) fn settings_sources() -> ConfigSources {
