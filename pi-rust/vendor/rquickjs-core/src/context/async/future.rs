    ...
    fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        let mut lock = loop {
            ...
            if let LockState::Pending(ref mut fut) = &mut this.lock_state {
                ...
                // At this point we have acquired a lock, so we no longer need the lock
                // future. `mem::replace` moves the old `LockState::Pending` variant out
                // *before* the new value is written, so the variant's `Drop` impl runs
                // exactly once. The previous code called `ManuallyDrop::drop(fut)` and
                // then assigned `LockState::Initial`, which dropped the very same
                // `Pending` variant a second time: `Drop for LockState` unconditionally
                // calls `ManuallyDrop::drop` again, double-freeing the `AcquireSlow`
                // `EventListener` owned by the mutex lock future.
                //
                // Upstream fix, verbatim from rquickjs-core 0.12.0:
                // "Replace lock_state with Initial first, so the old Pending variant is
                // moved out and its Drop runs exactly once."
