...
| `cargo test --workspace --locked` | **2506 passed / 0 failed / 2 ignored**（162 套件，本轮分支）；**合并 `feature/pi.rs` tip 后重跑 2520 passed / 0 failed / 2 ignored**（162 套件）|

上轮基线 2469 → **+37**（`scoped_models` 25 + 面板和弦/cycle/seed 8 + `config` 4 = 37）；
合并 tip 的 2520 = 2506 + LUM-1310 已入库的 14 个。合并后 `fmt` / `clippy` / `test` 三条与
两条 PTY 场景（28/28、12/12）都在**合并后的树**上重跑过。
