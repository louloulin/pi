...
| `pi-tui` 全量 | **1163 passed / 0 failed**（本轮新增 **17** 条；其余来自并入轮，见 §0.1/§0.2） | `cargo test -p pi-tui` |
| `pi-coding-agent` | **836 passed / 28 failed**（28 条与基线同集合，全为 Windows 环境类） | §4.3 |
| 纯代码规模（src↔src） | **93.7%**（143,507 / 153,106）——涨幅**主要来自本轮救回的交付**，本轮的 src 净增 **+256 行** | §4.4 |
| 测试规模 | **51.0%**（2,838 / 5,563） | §4.4 |
| `app.*` 接线 | **44/44 = 100%**（本轮并入 LUM-1263 后 silent 清零） | `app_action_coverage.py` |
| 加权完成度 | **87.1%**（86.9% → 87.07 → 87.1%；只动测试轴与接线率，两者都含并入轮） | §4.5 |
...
### 0.2 推送前再次并入 `origin/feature/pi.rs`（LUM-1457 已先落）

本轮推送前 `origin/feature/pi.rs` 已经从 `fc18cb09e` 走到 `c7a7b5878`
（`LUM-1457`：Windows 真 PTY 打通 + 「每次按键执行两遍」修复，+9 条测试标记）。
把它并入本轮分支时只有 `FEATURE_PI_RS_STATUS.md` 一处冲突（两侧各自追加一轮的段落），
按「两段都留、按并入顺序排列」解决（LUM-1457 在前、LUM-1469 在后），
`RUST_TS_PARITY_METRICS.md` 自动合并后把两侧的段号重排为 `§0.19/§0.20/§0.21/§0.22`。
...
|---|---|---|---|
| Rust src（去 mod.rs） | 143,507 行 / 254 文件 | 141,365 / 254 | **+2,092**，其中本轮救回的 LUM-1461 +1,012 行、LUM-1263 +387 行（两条各自文档口径），本轮自身 **+256 行**（`app.rs` +80/-2、`extension_ui.rs` +58/-1、`message.rs` +140/-19）；余量为两条交付与合并树的其它既有差异 |
| TS src | 153,106 行 / 669 文件 | 153,106 | 未变 |
| 规模比 | **93.7%** | 92.3% | **涨幅主要来自救回的两条交付，不是本轮新功能** |
| Rust `#[test]` | **2,838** | 2,773（issue 基线 `fc18cb09e`） | +65 = 救回交付 +39（`20f3666d9` = 2812）+ 并入的 LUM-1457 +9（`c7a7b5878` = 2782）+ 本轮 **+17** |
| TS 用例 | 5,563 | 5,563 | 同口径 |
| 测试比 | **51.0%** | 49.8% | 2838/5563 = 0.5102 |
...
| 12 | 测试与门禁强度 | 5% | **51.0%** | §4.4 |
| 13 | 子包完整度 | 3% | 95% | 未动 |

```
5×1.00 + 13×0.90 + 8×1.00 + 6×0.70 + 14×0.929 + 8×0.90 + 7×0.78 + 7×0.70
+ 8×0.95 + 7×1.00 + 9×0.85 + 5×0.5102 + 3×0.95 = 87.12% → **87.1%**
```

**口径声明（诚实读法）**：轴 5 的 +0.09 来自**并入的 LUM-1263**（`app.*` 44/44），
轴 12 里 LUM-1457 的 +9 条标记来自**并入的 LUM-1457**，都不是本轮写的代码；
本轮自己在这一轴上只贡献 `2812→2838`（+17 条，+0.06pt）与轴 6 的**证据修复**。
