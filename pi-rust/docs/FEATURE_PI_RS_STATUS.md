...
（Stage 35 实施轮，由 LUM-1126 协调轮派发。起点 `origin/feature/pi.rs` @ `c7dec41bc`；工作分支
`work/lum-1128`。只做 frontier 第 1 项：把 `App` 的鼠标手势从「全局」升级成「按矩形派发给命中的
模态」，并在点击命中时清掉文本选区。）
...
9. **不做**：双击选词 / 三击选行 / 边缘自动滚动 / grapheme 整格扩边（frontier 第 1 项，与 `app.rs`
   同文件）；`alt-screen-search.ts`（第 5 项）；X10 旧式鼠标序列（第 8 项）。
...
2. **P2 `node:module` / `node:readline`**（Stage 36 在跑）：`node_arg_bytes` / base64 helper 已就位。
3. **P2 `node:zlib` 的 gzip/deflate**（`gunzipSync` / `gzipSync` / `deflateSync` / `inflateSync`）：
   纯依赖问题，离线 registry 有 `flate2` / `miniz_oxide` 时再接（`host.rs` 的 `zlib.*` op 表与
   `tests/zlib.rs` 骨架可直接加 arm）。
4. **P2 `fetch` 全局**：`.pi/extensions/import-repro.ts` 只差它，要真实 HTTP 桥，落 `host.rs`/shim 通道。
5. **P3 `alt-screen-search.ts`**（上游 327 行）：与选区高亮有天然联动，需要 `app.rs` 钩子。
6. **P3 `latex.ts`**（1394 行）与 `markdown.rs` 未覆盖子集（表格 / LaTeX / OSC-8 hyperlink / 语法高亮）：
   `markdown.rs` + `lib.rs`，与第 1 项不同文件面，`lib.rs` 的 `pub mod` 需错开。
7. **P3 provider catalog / LUM-1090**：结论维持（无上游 `data/*.json` 事实源，不写猜测值）。
8. **P3 旧式 X10 鼠标序列、`updateScrollbarHover` 悬停高亮、滚动条拖拽**（原第 9 项，本轮**不再被
...
9. **新增欠账**：`/settings` 仍只有三条设置（上游 945 行的 `SettingsSelectorComponent` 还有
   steering/follow-up 模式、transport、`modelThinkingLevels`、图片处理等），随各自子系统补；以及
