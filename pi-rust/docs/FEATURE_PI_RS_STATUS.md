...
`c7dec41bc`；合并时远端已推进到 `ffe2a68cf`（含 LUM-1127 的 GFM 表格切片），本轮切片与它
**代码零冲突**（`markdown.rs` 与本轮文件面不同），只有本文档尾部两边同时在追加：已按「LUM-1127
之后」的顺序解决。只做 frontier 第 1 项：把 `App` 的鼠标手势从「全局」升级成「按矩形派发给命中的
模态」，并在点击命中时清掉文本选区。）
...
6. **P3 `latex.ts`**（1394 行）与 `markdown.rs` 的剩余子集：**表格**已由 LUM-1127 落地；剩下 LaTeX、
