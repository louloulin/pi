...
## LUM-1469 round — 排队输入的可见面（composer 上方 `Steering:`/`Follow-up:` 块 + `↳ <chord>` 取回提示）+ 并入两条 `in_review` 交付；`pi-tui` 1154/0；派发 1 条（槽位 2/3）
...
* `cargo test --offline -p pi-tui -j 8`：**1154 passed / 0 failed**
  （救回两条交付后的基线 `20f3666d9` = **1137/0** → 本轮 **+17**）。
